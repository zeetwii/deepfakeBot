...
| Above are the red and green histograms for the deepfake face ([the deepfake](./photos/test.png)).  You can see that they look very different from both our target and control faces, in fact they don't really look like the target face's histograms at all.  Instead they look like someone added a bunch of noise to the Control face, because thats exactly what the face replacement algorithm did when filling in the gaps and replacing the original face.  |
...
Audio deepfakes are interesting, because while they are also relatively old, they respond very well to things like model tagging and watermarking.  This is because human hearing can only hear a limited range of frequencies, and most audio deepfake models will add noise to the audio that is outside of this range.  Audio models actually have a long history of using these kinds of watermarks to identify synthetic audio, Google used to use a similar technique to identify audio from commercials to prevent them from activating their voice assistants.  Regardless of if you are adding or removing noise to create a watermark, one of the best ways to visualize and analyze audio deepfakes is to use spectrogram analysis.

## Spectrogram Analysis

Spectrograms are a common tool in audio analysis, and they are used to represent the frequency content of an audio signal over time. They are often used in speech recognition, music analysis, and other applications. In our case, we'll be using [Audacity](https://www.audacityteam.org/) to generate spectrograms of our audio samples.  These samples are both myself and the synthetic audio counting from one to ten.

| Spectrogram Analysis of Real and Synthetic Audio |
|:----------------------------------:|
| ![Real Audio Spectrogram](./photos/audio/sourceSpectrum.png) |
| This is the spectrogram of my real voice counting from one to ten.  While the spectrogram gets a little noisy at the higher frequencies, it is relatively smooth and continuous. |
| ![Synthetic Audio Spectrogram](./photos/audio/synthSpectrum.png) |
| This is the spectrogram of the synthetic audio generated by ElevenLabs of my voice counting from one to ten.  You can see that there are a lot of sharp spikes and valleys in the higher frequencies, which is a strong indicator that the audio is synthetic. |

Audio comparisons are a little more difficult to do than image comparisons, because you need to have the audio files to compare, and the recording and speaker quality can have a big impact on the analysis.  For example, here we are playing the synthetic audio through a speaker and recording it with a microphone, which adds a lot of noise to the signal.  However, even with this added noise, you can still see a lot of weird artifacts in the distribution of frequencies, which is a strong indicator that the audio is synthetic.  I haven't been able to find any documentation from ElevenLabs about if they add a watermark to their audio, but multiple posts on Reddit have indicated that they do.  More then likely, one of the spikes in the higher frequencies is the watermark, while the weird artifacts in the lower frequencies are from the cheap speaker I used to playback the audio.
